...
if ($UPN) {
    $Success = "Removed AzureAD\$UPN as local administrator." | Out-File -FilePath $env:TEMP\LocalAdmin.log
    if (!($Localadmins -contains $Username)) {
        Remove-LocalGroupMember -Group $Localadmingroupname -Member "Azuread\$UPN"
        $Success = "Removed AzureAD\$UPN as local administrator." | Out-File -FilePath $env:TEMP\LocalAdmin.log
    }
    else {
        $Alreadymember = "AzureAD\$UPN is not a local administrator." | Out-File -FilePath $env:TEMP\LocalAdmin.log
    }
}
else {
